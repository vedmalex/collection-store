...
└── b-plus-tree/
    ...
    ├── README.md
    ├── technical-debt-resolution-report.md
    └── vitest.config.ts
```
...
- [BPlusTree Adapter Guide](../integration/B_PLUS_TREE_ADAPTER.md)
...
`SPEC/spec_bplus_tree.ts`

```ts
/**
 * SPEC: BPlusTree Adapter and Compatibility Layer
 *
 * Context:
 * - Runtime error observed: "TypeError: BPlusTree.serialize is not a function"
 * - Different builds/versions/interop of 'b-pl-tree' expose different APIs.
 *
 * Goals:
 * - Provide stable (de)serialization across versions and ESM/CJS builds
 * - Maintain backward compatibility with existing portable formats
 * - Avoid test flakes and timeouts caused by index persistence
 */

export interface BPlusTreeLike<K = any, V = any> {
  insert(key: K, value: V): void;
  remove?(key: K): void;
  removeSpecific?(key: K, predicate: (v: V) => boolean): void;
  find?(key: K): Iterable<V> | V[];
  findFirst?(key: K): V | undefined;
  findLast?(key: K): V | undefined;
  each?(forward?: boolean): (self: unknown) => IterableIterator<{ key: K; value: V }>;
  min?: any;
  max?: any;
  size?: number;
}

export type PortableBPlusTree<K = any, V = any> = {
  __bpl_adapter?: 'v1';
  __bpl_version?: string;
  // Minimal required fields for rebuild fallback
  order?: number;
  unique?: boolean;
  // When static serialize exists, this will be vendor-specific payload
  payload?: unknown;
  // Generic fallback payload (inorder snapshot)
  snapshot?: Array<{ key: K; value: V }>;
};

export type BPlusTreeAdapter = {
  /** Try all known static/instance strategies, otherwise fallback snapshot */
  serializeTree: (tree: unknown) => PortableBPlusTree;
  /** Try vendor createFrom/deserialize, otherwise rebuild by inserting snapshot */
  deserializeTree: (portable: PortableBPlusTree) => BPlusTreeLike;
  /** Resolve BPlusTree class constructor from module namespace */
  resolveBPlusTreeClass: () => any;
  /** Runtime capability checks for static API */
  isStaticSupported: (klass: any) => boolean;
};

/**
 * Implementation notes (to be placed in src/internal/bpl/BPlusTreeAdapter.ts):
 *
 * - Resolve module in a resilient way:
 *   import * as NS from 'b-pl-tree'
 *   const BPlusTree = NS.BPlusTree ?? NS.default?.BPlusTree ?? NS.default ?? NS
 *
 * - Serialization strategy:
 *   1) if (typeof BPlusTree.serialize === 'function') -> use it
 *   2) else if (typeof tree.constructor?.serialize === 'function') -> use it
 *   3) else if (typeof (tree as any).serialize === 'function') -> call instance
 *   4) else -> build PortableBPlusTree with snapshot via iterator (if present)
 *
 * - Deserialization strategy:
 *   1) if (typeof BPlusTree.createFrom === 'function') -> return BPlusTree.createFrom(portable)
 *   2) else if (typeof BPlusTree.deserialize === 'function') -> return BPlusTree.deserialize(new BPlusTree(), portable)
 *   3) else if (portable.snapshot) -> const t = new BPlusTree(); forEach insert; return t
 *   4) else throw BPL_ADAPTER_DESERIALIZE_UNSUPPORTED
 *
 * - Error handling:
 *   Throw errors with codes: 'BPL_ADAPTER_SERIALIZE_UNSUPPORTED' / 'BPL_ADAPTER_DESERIALIZE_UNSUPPORTED'
 *
 * - Backward compatibility:
 *   If vendor payload exists, store it in 'payload' so older readers can ignore adapter fields.
 */

/**
 * Test Matrix (unit):
 * - Vendor A: has static serialize/deserialize
 * - Vendor B: has static serialize/createFrom
 * - Vendor C: only instance.serialize()
 * - Vendor D: no methods -> snapshot fallback (rebuild)
 *
 * Integration:
 * - Collection.store()/restore() -> AdapterFile store/restore -> indexes ok
 * - FileStorage clone/persist/load round-trip
 */

```

`src/internal/bpl/BPlusTreeAdapter.ts`

```ts
/*
 * BPlusTree Adapter: resilient (de)serialization across ESM/CJS and versions
 * Code and comments are in English per project guidelines.
 */

import * as BTreeNS from 'b-pl-tree';
import { Logger } from '../../logger';

// Local re-exports and types to align with SPEC without leaking into public API
export type BPlusTreeLike<K = any, V = any> = {
  insert: (key: K, value: V) => void;
  remove?: (key: K) => void;
  removeSpecific?: (key: K, predicate: (v: V) => boolean) => void;
  find?: (key: K) => Iterable<V> | V[];
...
export type PortableBPlusTree<K = any, V = any> = {
  ...
  order?: number;
  unique?: boolean;
  payload?: unknown; // vendor payload when static serialize exists
  snapshot?: Array<{ key: K; value: V }>; // generic fallback snapshot
  // Allow legacy vendor formats to pass through unchanged
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [key: string]: any;
};

const ERROR_SERIALIZE = 'BPL_ADAPTER_SERIALIZE_UNSUPPORTED';
const ERROR_DESERIALIZE = 'BPL_ADAPTER_DESERIALIZE_UNSUPPORTED';

type BPlusTreeCtor = new (...args: Array<any>) => BPlusTreeLike;

export function resolveBPlusTreeClass(): BPlusTreeCtor {
  // Test hook to allow overriding resolution in unit tests
  const hook = (globalThis as any).__BPL_ADAPTER_RESOLVE__;
  if (typeof hook === 'function') {
    const hooked = hook();
    if (hooked) return hooked as BPlusTreeCtor;
  }
  // Resolve constructor from namespace with robust fallbacks
  // Prefer explicit exported class, then default variant, then namespace itself
  const cls = (BTreeNS as any).BPlusTree
    ?? (BTreeNS as any).default?.BPlusTree
    ?? (BTreeNS as any).default
    ?? (BTreeNS as any);
  return cls as BPlusTreeCtor;
}

export function isStaticSupported(klass: any): boolean {
  return (
    typeof klass?.serialize === 'function' ||
    typeof klass?.deserialize === 'function' ||
    typeof klass?.createFrom === 'function'
  );
}

export function serializeTree(tree: unknown): PortableBPlusTree {
  const Klass = resolveBPlusTreeClass();

  // Try static serialize on resolved class
  if (typeof (Klass as any)?.serialize === 'function') {
    try {
      Logger.debug('BPL Adapter: using static serialize');
      const payload = (Klass as any).serialize(tree);
      return { __bpl_adapter: 'v1', __bpl_version: getVersionSafe(), payload };
    } catch (err) {
      Logger.warn('BPL Adapter: static serialize failed, falling back', err);
    }
  }

  // Try static serialize on constructor of instance
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ctor = (tree as any)?.constructor;
  if (typeof ctor?.serialize === 'function') {
    try {
      Logger.debug('BPL Adapter: using constructor static serialize');
      const payload = ctor.serialize(tree);
      return { __bpl_adapter: 'v1', __bpl_version: getVersionSafe(), payload };
    } catch (err) {
      Logger.warn('BPL Adapter: constructor static serialize failed, falling back', err);
    }
  }

  // Try instance serialize method
  if (typeof (tree as any)?.serialize === 'function') {
    try {
      Logger.debug('BPL Adapter: using instance serialize');
      const payload = (tree as any).serialize();
      return { __bpl_adapter: 'v1', __bpl_version: getVersionSafe(), payload };
    } catch (err) {
      Logger.warn('BPL Adapter: instance serialize failed, falling back', err);
    }
  }

  // Fallback: snapshot using iterator if exists
  const snapshot = snapshotFromIterator(tree);
  if (snapshot) {
    Logger.debug('BPL Adapter: using fallback snapshot');
    return { __bpl_adapter: 'v1', __bpl_version: getVersionSafe(), snapshot };
  }

  const error = new Error('Serialization not supported by adapter');
  (error as any).code = ERROR_SERIALIZE;
  throw error;
}

export function deserializeTree(portable: PortableBPlusTree): BPlusTreeLike {
  const Klass = resolveBPlusTreeClass();

  // Primary path: createFrom
  if (typeof (Klass as any)?.createFrom === 'function') {
    try {
      Logger.debug('BPL Adapter: using static createFrom');
      return (Klass as any).createFrom(portable.payload ?? portable);
    } catch (err) {
      Logger.warn('BPL Adapter: createFrom failed, trying other paths', err);
    }
  }

  // Secondary: deserialize(target, data)
  if (typeof (Klass as any)?.deserialize === 'function') {
    try {
      Logger.debug('BPL Adapter: using static deserialize');
      const instance = new (Klass as any)();
      (Klass as any).deserialize(instance, portable.payload ?? portable);
      return instance as BPlusTreeLike;
    } catch (err) {
      Logger.warn('BPL Adapter: static deserialize failed, trying fallback', err);
    }
  }

  // Fallback rebuild from snapshot
  if (Array.isArray(portable?.snapshot)) {
    Logger.debug('BPL Adapter: rebuilding from snapshot');
    const instance = new (Klass as any)();
    for (const { key, value } of portable.snapshot) {
      (instance as any).insert(key, value);
    }
    return instance as BPlusTreeLike;
  }

  const error = new Error('Deserialization not supported by adapter');
  (error as any).code = ERROR_DESERIALIZE;
  throw error;
}

function getVersionSafe(): string | undefined {
  try {
    // Attempt to read version from resolved namespace
    const v = (BTreeNS as any)?.version || (BTreeNS as any)?.default?.version;
...
function snapshotFromIterator(tree: unknown): Array<{ key: any; value: any }> | undefined {
  // Prefer each(forward?) → generator
  if (typeof (tree as any)?.each === 'function') {
    try {
      const iteratorFactory = (tree as any).each(true);
      const it = iteratorFactory(tree);
      const acc: Array<{ key: any; value: any }> = [];
      for (const item of it) {
        acc.push({ key: (item as any).key, value: (item as any).value });
      }
      return acc;
    } catch (err) {
      Logger.warn('BPL Adapter: iterator snapshot failed', err);
    }
  }

  // As a last-ditch option, try known methods to traverse contents
  // If no traversal is available, return undefined to signal unsupported
  return undefined;
}
...
> Adapter compatibility: Starting from 1.3.x, a resilient adapter layer normalizes (de)serialization across ESM/CJS and version differences. See Integration Guide: [B_PLUS_TREE_ADAPTER.md](./integration/B_PLUS_TREE_ADAPTER.md).
...
- **Почему вижу “BPlusTree.serialize is not a function”?**
  Вероятно, у вас смешанный ESM/CJS импорт или отличается версия экспортов. Используйте `serializeTree`/`deserializeTree` — адаптер выберет корректную стратегию.

- **Стало медленно при восстановлении?**
  Проверьте логи с `DEBUG_BTREE=true`. Если используется fallback‑rebuild — убедитесь, что доступен статический `createFrom`/`deserialize` или используйте актуальную версию библиотеки.

- **Можно ли читать старые JSON‑дампы?**
  Да. Поле `payload` поддерживается, при записи добавляются метаданные адаптера.

- **Где посмотреть детали по адаптеру?**
  См. `integration/B_PLUS_TREE_ADAPTER.md`.
