...
export default class AdapterLocalStorage<T extends Item> implements StorageAdapter<T>{
  storage: Storage
  ...
  store(name?: string): Promise<void> {
    return new Promise((res, rej) => {
      try {
        res();
        this.storage.setItem(this.collection.model, JSON.stringify(name ?? this.collection.store()));
      } catch (e) {
        rej(e)
      }
    })
