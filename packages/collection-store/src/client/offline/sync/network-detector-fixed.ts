/**
 * Phase 5.3: Offline-First Support - Network Detector (Fixed)
 ...
 * ✅ FIXED: Environment detection for Node.js/browser compatibility
 */
export class NetworkDetectorFixed implements INetworkDetector {
  ...
  private currentNetworkInfo: NetworkInfo = {
    isOnline: this.isBrowserEnvironment() ? (navigator as any).onLine : false,
    quality: 'offline',
  ...
  async getEfficiencyScore(): Promise<number> {
    ...
    // Quality contribution (0-30 points)
    switch (networkInfo.quality) {
      case 'excellent':
        score += 30;
        break;
    ...
    if (networkInfo.latency !== undefined) {
      if (networkInfo.latency < 50) {
        score += 20;
      } else if (networkInfo.latency < 200) {
        score += 10;
  ...
  async getStats(): Promise<{
    ...
    return {
      ...
      onlineTime: uptime * 0.8, // Placeholder calculation
      offlineTime: uptime * 0.2, // Placeholder calculation
  ...
  async import(data: {
    ...
    if (data.history) {
      this.networkHistory = data.history;
    }
  ...
  private async checkNetworkStatus(): Promise<void> {
    ...
    try {
      const isOnline = this.isBrowserEnvironment() ? (navigator as any).onLine : false;
